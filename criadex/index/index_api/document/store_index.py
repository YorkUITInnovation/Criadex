...
from criadex.index.schemas import CriadexBaseIndex, Bundle
from criadex.schemas import IndexType


class DocumentVectorStoreIndex(CriadexBaseIndex):
    """
    ...
    def seed_bundle(cls) -> Bundle:
        ...
        return Bundle(
            name="seed-file",
            config=DocumentConfig(nodes=[Element(type=ElementType.NARRATIVE_TEXT, text="seed-text")]),
            group=GroupsModel(
                id=-1,
                name="seed-group",
                type=IndexType.DOCUMENT.value,
                llm_model_id=-1,
